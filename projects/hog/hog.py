...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        temp = dice()
#        print("current point:",temp)
        sum += temp
        if temp ==1:
            flag_one = 1
#        print("current flag:",flag_one)
        num_rolls -= 1
    return sum if flag_one ==0 else flag_one
...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        result = player_score
        print("DEBUG:", "initial score:", result)
        while opponent_score > 0:
            digit = opponent_score%10
            print("DEBUG:", "digit:",digit) 
            result, opponent_score = digit_fn(digit)(result), opponent_score//10
            print("DEBUG:", "score after update:", result)
        return result%30
    # END PROBLEM 2
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    return player_score%10 if player_score%10 == opponent_score%10 else 0
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        player_score = get_scores(who)
        opponent_score = get_scores(next_player(who))
        num_rolls = get_strategy()(player_score, opponent_score)
        score = take_turn(num_rolls,player_score, opponent_score, dice, goal)
        update_scores(score) #n update the scores first to check hog pile
        update_scores(hog_pile(get_scores(who), opponent_score)) #update hog-pile if necessary 
        who = next_player(who)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    print("DEBUG: ","check leader: ", leader)
    announcement = f"Player {leader} takes the lead by {abs(score0-score1)}"
    # announcement = player + str(leader) + lead_message + str(abs(score0-score1))
    if last_leader is not None:
        announcement = announcement if last_leader != leader else None
        leader = leader if score0 != score1 else None
    print("DEBUG: ", "check announcement: ", announcement)
    return leader, announcement
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average(*args):
        ...
        while k <=total_samples:
            ...
            result += temp
            # print("DEBUG: ", f"original function return: ", temp)
            # print("DEBUG: ", f"result: {result}")
            k += 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    k = 1
    while k <= 10:
        succ = make_averaged(roll_dice, total_samples)(k, dice)
        num = k if succ > curr else num
        curr = succ 
        k += 1
    return num
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    updated_scores = score + hefty_hogs(score, opponent_score)
    if hog_pile(updated_scores, opponent_score) > 0:
        return 0
